...
impl<T> CircularBuffer<T> {
    pub fn new(capacity: usize) -> Self {
        Self {
            buffer: std::iter::repeat_with(|| None).take(capacity).collect(),
            len: 0,
            capacity,
            write_index: 0,
            read_index: 0,
    ...
    pub fn read(&mut self) -> Result<T, Error> {
        ...
        let value = self.replace_value(self.read_index, None);
        self.len -= 1;
    ...
    pub fn overwrite(&mut self, _element: T) {
        self.replace_value(self.write_index, Some(_element));
        self.advance_write_index();
        if self.is_full() {
            self.advance_read_index();
        } else {
    ...
    fn replace_value(&mut self, index: usize, value: Option<T>) -> Option<T> {
