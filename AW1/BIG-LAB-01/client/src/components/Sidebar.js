    ...
    <Collapse in={props.openCollapse}>
      <Col md={4} className="d-md-block bg-light pt-nav" id="sidebar">
        <ListGroup variant="flush" className="py-3">
          {Object.keys(props.filters).map((filter, idx) =>
            <FilterItem key={idx} filter={filter} selectedFilter={props.selectedFilter} />
          )}
        </ListGroup>
      </Col>
    </Collapse>
...
function FilterItem(props) {
  return (
    <Link className="text-link" to={`/tasks/${props.filter}`}>
